...
st.title('Pronostici partite ESP LEAUGUE con il Machine Learning')
...
def load_dataframes():
    ...
    for year in range(5, 24):
        url = f'https://www.football-data.co.uk/mmz4281/{year:02d}{year + 1:02d}/SP1.csv'
...
url = 'https://fbref.com/it/comp/12/calendario/Risultati-e-partite-di-La-Liga'
...
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Athletic Club', 'Ath Bilbao')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Athletic Club', 'Ath Bilbao')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Celta Vigo', 'Celta')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Celta Vigo', 'Celta')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Alavés', 'Alaves')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Alavés', 'Alaves')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Leganés', 'Leganes')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Leganés', 'Leganes')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Real Sociedad', 'Sociedad')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Real Sociedad', 'Sociedad')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Rayo Vallecano', 'Vallecano')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Rayo Vallecano', 'Vallecano')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Espanyol', 'Espanol')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Espanyol', 'Espanol')
df_forecast['HomeTeam'] = df_forecast['HomeTeam'].replace('Atlético Madrid', 'Ath Madrid')
df_forecast['AwayTeam'] = df_forecast['AwayTeam'].replace('Atlético Madrid', 'Ath Madrid')
